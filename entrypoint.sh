...
echo $(/composer/vendor/bin/ecs check $1 --clear-cache --output-format=json > one.json)
echo $(/composer/vendor/bin/ecs check $1 --clear-cache --output-format=json > two.json)
echo $(/composer/vendor/bin/ecs check $1 --clear-cache --output-format=json > thr.json)
